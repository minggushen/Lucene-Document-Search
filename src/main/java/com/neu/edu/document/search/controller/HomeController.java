...
public class HomeController {
	...
	public String getSearchedWords(Map<String,Object> map,String searchWord) throws Exception {
		ArrayList<HighlightInformation> resultsFetched = new ArrayList<HighlightInformation>();

		logger.debug("[welcomeName] search : {}", counter);

		ArrayList<HighlightInformation> displayData = SearchHighlighter.getDoc(searchWord, resultsFetched);

		if (displayData.size() != 0) {
			logger.debug("search highlight", SEARCHED_PAGE);
			if (displayData.size() > 0) {
				map.put("foundResults",displayData);
